...
def totalCpuTime(dev):
    ...
    for info in res:
        if info == "cpu":
            ...
            logger.info("nice = %s" % nice)
            logger.info("system = %s" % system)
            logger.info("idle = %s" % idle)
            logger.info("iowait =%s" % iowait)
            logger.info("irq = %s" % irq)
            logger.info("softirq = %s" % softirq)"""
            result = int(user) + int(nice) + int(system) + int(idle) + int(iowait) + int(irq) + int(softirq)
            #logger.info("totalCpuTime=" + str(result))
            return result
...
def processCpuTime(pid, dev):
    ...
    try:
        ...
        cutime = res[15]
        cstime = res[16]
        """logger.info("utime = %s" % utime)
        logger.info("stime = %s" % stime)
        logger.info("cutime = %s" % cutime)
        logger.info("cstime = %s" % cstime)"""
        result = int(utime) + int(stime) + int(cutime) + int(cstime)
        #logger.info("processCpuTime=" + str(result))
    except:
        result = 0
    return result
...
def cpu_rate(pid, cpukel, dev):
    ...
    #logger.info("totalCpuTime3=" + str(totalCpuTime3))
    #logger.info("processCpuTime3=" + str(processCpuTime3))
