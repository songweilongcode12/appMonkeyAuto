# -*- coding: utf-8 -*-
import shutil
from Base.BasePickle import writeInfo, writeSum, readInfo, AdbCommon
...
from Base import BaseMonitor, BasePhoneMsg
import threading
...
def start(dev):
    ...
    if signal == 'y':
        logger.info("测试即将开始，请打开需要测试的app并准备执行您的操作....")
        time.sleep(5)
        path_log = Config.log_location + dev
        device_dir = os.path.exists(path_log)
        if device_dir:
            logger.info("日志文件目录log已存在，继续执行测试!")
        else:
        ...
        logger.info("测试前电量", beforeBattery)
        try:
            while True:
    ...
    if readInfo(PATH("./info/sumInfo.pickle")) <= 0:
        ...
        logger.info("Kill adb server,test finished！")
        os.popen("taskkill /f /t /im adb.exe")
        shutil.rmtree((PATH("./info/"))) # 删除持久化目录

    elif signal == 'n':
        logger.info('用户主动放弃测试，测试结束！')
    else:
        logger.info("测试结束，输入非法，请重新输入y or n！")"""

    #Monkey测试部分，如果是进行monkey测试，去除该部分注释（line134~line191）并注掉手动测试部分(line72~line131)；另外BaseReport.py中line85，line120~line132注释也要去除
    logger.info("--------------开始执行Monkey----------------")
    path_log = Config.log_location +time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))
    device_dir = os.path.exists(path_log)
    if device_dir:
        logger.info("日志文件目录log已存在，继续执行测试!")
    else:
    ...
    os.popen(cmd_monkey)
    logger.info("monkey 命令:", cmd_monkey)
...
if __name__ == '__main__':
    ...
    if device_dir:
        logger.info("持久性目录info已存在，继续执行测试!")
    else:
